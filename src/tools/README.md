...
5. Install with `betterclaw tool install path/to/tool.wasm`
...
When running `betterclaw tool auth <tool>`:
...
Both are first-class in the extension system (`betterclaw tool install` handles both), but they have different strengths.

**WASM Tools (BetterClaw native)**
...
- Cost: external process with full system access (no sandbox), manages own credentials, BetterClaw can't prevent leaks
