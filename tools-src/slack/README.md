...
A standalone WASM component that provides Slack integration for BetterClaw. This serves as both a functional tool and a template for building custom WASM tools.
...
```bash
mkdir -p ~/.betterclaw/tools
cp target/wasm32-wasip2/release/slack_tool.wasm ~/.betterclaw/tools/slack.wasm
cp slack.capabilities.json ~/.betterclaw/tools/
```
...
```bash
betterclaw secret set slack_bot_token "xoxb-your-token-here"
```
...
```bash
betterclaw secret set slack_bot_token "xoxb-..."
```
