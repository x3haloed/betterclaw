...
use betterclaw::llm::{LlmProvider, SessionConfig, SessionManager};
use betterclaw::tools::Tool;
...
use betterclaw::llm::recording::{HttpExchange, ReplayingHttpInterceptor};
...
pub struct TestRig {
    ...
    #[cfg(feature = "libsql")]
    workspace: Option<Arc<betterclaw::workspace::Workspace>>,
...
impl TestRig {
    ...
    pub async fn send_incoming(&self, msg: betterclaw::channels::IncomingMessage) {
    ...
    pub fn captured_llm_requests(&self) -> Vec<Vec<betterclaw::llm::ChatMessage>> {
        self.trace_llm
            .as_ref()
...
impl TestRigBuilder {
    ...
    pub async fn build(self) -> TestRig {
        use betterclaw::channels::ChannelManager;
        use betterclaw::db::libsql::LibSqlBackend;
        ...
        let db: Arc<dyn betterclaw::db::Database> = Arc::new(backend);
        ...
        // 6. Register job tools, routine tools, and extra tools.
        {
            ...
            if let (Some(db_arc), Some(ws)) = (&components.db, &components.workspace) {
                use betterclaw::agent::routine_engine::RoutineEngine;
                use betterclaw::config::RoutineConfig;
            ...
            if enable_skills {
                let registry = Arc::new(std::sync::RwLock::new(
                    betterclaw::skills::SkillRegistry::new(temp_dir.path().join("skills"))
                        .with_installed_dir(temp_dir.path().join("installed_skills")),
                ));
                let catalog = betterclaw::skills::catalog::shared_catalog();
                components
                    .tools
            ...
            http_interceptor: {
                ...
                if exchanges.is_empty() {
                    ...
                    Some(Arc::new(ReplayingHttpInterceptor::new(exchanges))
                        as Arc<dyn betterclaw::llm::recording::HttpInterceptor>)
                }
            },
            transcription: None,
            document_extraction: None,
            ledger_index_config: components.config.ledger_index.clone(),
            ledger_recall_config: components.config.ledger_recall.clone(),
        };
        ...
        let routine_config = if enable_routines {
            Some(betterclaw::config::RoutineConfig {
                enabled: true,
...
impl TestRig {
    ...
    #[cfg(feature = "libsql")]
    pub fn workspace(&self) -> Option<&Arc<betterclaw::workspace::Workspace>> {
