...
//! OAuth flow. Tokens are stored in `~/.betterclaw/session.json` and refreshed
...
pub struct SessionConfig {
    ...
    /// Path to session file (e.g., ~/.betterclaw/session.json).
...
impl SessionManager {
    ...
    /// saved to `~/.betterclaw/.env` for persistence across restarts.
    ...
    async fn api_key_login(&self) -> Result<(), LlmError> {
        ...
        // Persist to ~/.betterclaw/.env so the key survives restarts
