...
fn read_attachments(paths: &[String]) -> Result<Vec<wit_channel::Attachment>, String> {
    ...
    let home_base = dirs::home_dir()
        .map(|h| h.join(".betterclaw"))
        .unwrap_or_default();

    for path in paths {
        // Validate paths are under /tmp/ or ~/.betterclaw/ to prevent arbitrary file reads
        ...
        let validated = validated.map_err(|e| {
            format!(
                "Invalid attachment path '{}': must be under /tmp/ or ~/.betterclaw/: {}",
                path, e
            )
        })?;
...
mod tests {
    ...
    fn test_mime_from_extension() {
        ...
        assert_eq!(
            mime_from_extension("/home/user/.betterclaw/screenshot.png"),
            "image/png"
        );
    }
}
