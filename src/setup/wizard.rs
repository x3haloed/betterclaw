...
impl SetupWizard {
    ...
    async fn reconnect_existing_db(&mut self) -> Result<(), SetupError> {
        ...
        Err(SetupError::Database(
            "No database configured. Run full setup first (betterclaw onboard).".to_string(),
        ))
    }
    ...
    async fn reconnect_postgres(&mut self) -> Result<(), SetupError> {
        let url = std::env::var("DATABASE_URL").map_err(|_| {
            SetupError::Database(
                "DATABASE_URL not set. Run full setup first (betterclaw onboard).".to_string(),
            )
        })?;
    ...
    async fn test_database_connection_postgres(&mut self, url: &str) -> Result<(), SetupError> {
        ...
        if major_version < MIN_PG_MAJOR_VERSION {
            return Err(SetupError::Database(format!(
                "PostgreSQL {} detected. BetterClaw requires PostgreSQL {} or later for pgvector support.\n\
        ...
        if pgvector_row.is_none() {
            return Err(SetupError::Database(format!(
                "pgvector extension not found on your PostgreSQL server.\n\n\
                 ...
                 Then restart PostgreSQL and re-run: betterclaw onboard",
                major_version
            )));
        }
    ...
    async fn step_security(&mut self) -> Result<(), SetupError> {
        ...
        match choice {
            ...
            1 => {
                ...
                // Store hex for write_bootstrap_env to persist to ~/.betterclaw/.env.
                self.settings.secrets_master_key_hex = Some(key_hex.clone());

                println!();
                print_info("Master key generated and will be saved to ~/.betterclaw/.env");
                println!();
    ...
    async fn auto_setup_security(&mut self) -> Result<(), SetupError> {
        ...
        self.settings.secrets_master_key_source = KeySource::Env;
        print_success("Master key stored in ~/.betterclaw/.env");
        Ok(())
    }
    ...
    async fn save_and_summarize(&mut self) -> Result<(), SetupError> {
        ...
        if self.config.quick {
            print_info(
                "Tip: Run `betterclaw onboard` to configure channels, extensions, embeddings, and more.",
            );
            println!();
        }
...
mod tests {
    ...
    async fn test_discover_wasm_channels_nonexistent_dir() {
        let channels = discover_wasm_channels(
            &std::env::temp_dir().join("betterclaw_nonexistent_dir_abcxyz123"),
        )
        .await;
        assert!(channels.is_empty());
    }
