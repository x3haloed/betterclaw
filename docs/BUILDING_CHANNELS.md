...
This guide covers how to build WASM channel modules for BetterClaw.
...
version = "0.1.0"
edition = "2021"
description = "My messaging platform channel for BetterClaw"
...
**Do not commit compiled WASM binaries.** They are a supply chain risk — the binary in a PR may not match the source. BetterClaw builds channels from source:
...
# Install (or use betterclaw onboard to install bundled channel)
...
**Note**: The main BetterClaw binary bundles `telegram.wasm` via `include_bytes!`. When modifying the Telegram channel source, run `./channels-src/telegram/build.sh` **before** building the main crate, so the updated WASM is included.
...
# Deploy to ~/.betterclaw/channels/
cp target/wasm32-wasip2/release/my_channel.wasm ~/.betterclaw/channels/my-channel.wasm
cp my-channel.capabilities.json ~/.betterclaw/channels/
```
