...
mod tests {
    ...
    async fn system_event_trigger_matches_and_filters() {
        ...
        filters.insert("repository".to_string(), "nearai/betterclaw".to_string());
        ...
        let fired_wrong_type = engine
            .emit_system_event(
                "github",
                "issue.closed",
                &serde_json::json!({"repository": "nearai/betterclaw"}),
                Some("default"),
            )
            .await;
        assert_eq!(
    ...
    async fn heartbeat_findings() {
        ...
        match result {
            betterclaw::agent::HeartbeatResult::NeedsAttention(msg) => {
                assert!(
                    msg.contains("error"),
    ...
    async fn heartbeat_empty_skip() {
        ...
        let result = runner.check_heartbeat().await;
        assert!(
            matches!(result, betterclaw::agent::HeartbeatResult::Skipped),
