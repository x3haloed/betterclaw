...
mod spot_tests {
    ...
    async fn spot_chain_write_read() {
        let _cleanup = CleanupGuard::new().file("/tmp/betterclaw_spot_test.txt");
        let _ = std::fs::remove_file("/tmp/betterclaw_spot_test.txt");
        ...
        rig.send_message(
            "Write the text 'betterclaw spot check' to /tmp/betterclaw_spot_test.txt \
        ...
        let content =
            std::fs::read_to_string("/tmp/betterclaw_spot_test.txt").expect("file should exist");
        assert_eq!(content, "betterclaw spot check");
